    ...
    if (!isset($_SESSION["uid"])){
        ...
        function setAnswers(){
            try{
                ...
                $sql1 =  "SELECT * FROM options WHERE uid = '". $uid."'";
                ...
                if (isset($_POST['update-options'])){
                    ...
                    $option4 = $_POST["submitOptions"];

                    print_r($option1);

                    if ($isInTable){
                        // update database
                        $update1 = "UPDATE `options` SET `virtueEthicsOptions`= '".$option1."' WHERE name = '".$namearray[0]."' and uid = ".$uid."";
                        $pdo -> query($update1);

                        $update2 = "UPDATE `options` SET `virtueEthicsOptions`= '".$option2."' WHERE name = '".$namearray[1]."' and uid = ".$uid."";
                        $pdo -> query($update2);

                        $update3 = "UPDATE `options` SET `virtueEthicsOptions`= '".$option3."' WHERE name = '".$namearray[2]."' and uid = ".$uid."";
                        $pdo -> query($update3);

                        $update4 = "UPDATE `options` SET `virtueEthicsOptions`= '".$option4."' WHERE name = '".$namearray[3]."' and uid = ".$uid."";
                        $pdo -> query($update4);
                        ...
                        $insert1 = "INSERT INTO `options`(`virtueEthicsOptions`) VALUES (".$option1.") WHERE name = '".$namearray[0]."' and uid = ".$uid."";
                        $pdo -> query($insert1);

                        $insert2 = "INSERT INTO `options`(`virtueEthicsOptions`) VALUES (".$option2.") WHERE name = '".$namearray[1]."' and uid = ".$uid."";
                        $pdo -> query($insert2);

                        $insert3 = "INSERT INTO `options`(`virtueEthicsOptions`) VALUES (".$option3.") WHERE name = '".$namearray[2]."' and uid = ".$uid."";
                        $pdo -> query($insert3);

                        $insert4 = "INSERT INTO `options`(`virtueEthicsOptions`) VALUES (".$option4.") WHERE name = '".$namearray[3]."' and uid = ".$uid."";
                        $pdo -> query($insert3);
